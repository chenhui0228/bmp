...
def translate_date( sub, start_time, every, weekday ):
    ...
    dict = {'run_sub': 'date', 'year': '*', 'month': '*', 'day': '*', 'week': '*', 'day_of_week': '*', 'hour': '*',
            'minute': '*', 'second': '*', 'start_date': '%s' % str(time_local)}
    if sub == 'once':
        pass
    elif sub == 'hourly':
    ...
    def __call__( self, message_dict ):
        ...
        if typeofMessage == 'first':
            ...
            try:
                ...
                bk_value['message'] = dict.get(
                    'message') + 'you get the message that the backup_state is finished before it created,there must be sometings wrong'
                bk_value['state'] = dict.get('state')
                try:
                    bk = self.db.bk_create(super_context, bk_value)
                except Exception as e:
...
class Server:
    ...
    def update_task( self, id, isRestart=False ):
        ...
        if len(new_vor_dir) == 0:
            ...
            vol = new_vor_dir[0]
            dir = new_vor_dir[1]
        dict = translate_date(policy.recurring, task.start_time, policy.recurring_options_every,
                              policy.recurring_options_week)
